...
# show_progress=True; numberOfStateSamples=1000; numberOfThetaSamples=1000; coefficient = .1; import generate_task as gt; td = gt.generate_task(300)
def SMC2(td, show_progress=True, numberOfStateSamples=1000, numberOfThetaSamples=1000, coefficient = .5):
    ...
    # Decode correct action for each trial, denotes all the correct actions
    # correctActions = np.zeros(numberOfTrials) 
    # for trial_idx in np.arange(numberOfTrials): correctActions[trial_idx] = mapping[stimuli[trial_idx], Z_true[trial_idx]]
    ...
    tauAll      = np.zeros([numberOfTrials, numberOfThetaSamples])
    weightsAll  = np.zeros([numberOfTrials, numberOfThetaSamples])

    latentAll        = np.zeros([numberOfTrials, numberOfStateSamples])
    weightslatentAll = np.zeros([numberOfTrials, numberOfStateSamples])
    ...
    for T in range(numberOfTrials):
        ...
        if T > 0:
            ...
            ancestorSamples = np.array(currentSamples)             # Copy objet currentSamples -> no reference cf : https://openclassrooms.com/courses/apprenez-a-programmer-en-python/portee-des-variables-et-references
        ...
        if (essList[T] < coefficient * numberOfThetaSamples) and (acceptance_list[-1] > 0.05):
            ...
            for theta_idx in range(numberOfThetaSamples):
                ...
                # priorsLogRatio                 = useful_functions.log_beta_pdf(tauCandidate, tauPrior[0], tauPrior[1]) + useful_functions.log_beta_pdf(betaCandidate, betaPrior[0], betaPrior[1]) + useful_functions.log_dirichlet_pdf(gammaCandidate, gammaPrior) - \
                #                                         useful_functions.log_beta_pdf(tauSamples[idx_traj], tauPrior[0], tauPrior[1]) - useful_functions.log_beta_pdf(betaSamples[idx_traj], betaPrior[0], betaPrior[1]) - useful_functions.log_dirichlet_pdf(gammaSamples[idx_traj], gammaPrior)
        ...
        tauAll[T]           = tauSamples
        weightsAll[T]       = normalisedThetaWeights
        latentAll[T]        = currentSamples[0]
        weightslatentAll[T] = ancestorsWeights[0]
        ...
        # Update performance
        if (actions[T] == mapping[stimuli[T].astype(int), Z_true[T].astype(int)]):
        ...
        if show_progress:
            ...
            #plt.plot(x, normlib.pdf(x, np.mean(tauSamples), np.std(tauSamples)), 'b'); plt.hold(True)
            #plt.plot([np.mean(tauSamples), np.mean(tauSamples)], plt.gca().get_ylim(),'b', linewidth=2)
            #plt.plot([td['tau'], td['tau']], plt.gca().get_ylim(), 'b--', linewidth=2)
    ...
    return [td, tauAll, weightsAll, latentAll, weightsAll, tauSamples, volTracking, volStdTracking, betaSamples, betaTracking, betaStdTracking, gammaSamples, tsProbability, countPerformance, actions, acceptance_list, essList, time_list, elapsed_time]
