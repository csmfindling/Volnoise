...
# show_progress=False; lambdaa=2.; eta=.01; numberOfStateSamples=50; numberOfThetaSamples=50; coefficient = .5; import generate_task as gt; td = gt.generate_task(action_num = 2, state_num = 1)
def SMC2(td, show_progress=True, lambdaa=.9, eta=0., inertie_noise=0., numberOfStateSamples=2000, numberOfThetaSamples=1000, coefficient = .5, beta_softmax = None , espilon_softmax=0.):
    ...
    #etaPrior       = np.array([3, 7])              # np.array([.5, 1])
    ...
    # Z_true = np.zeros(numberOfTrials)
    # index = 0
    # ts = numpy.ones(K);
    # for trial_idx in np.arange(1,numberOfTrials):
    #     if Z[trial_idx] == Z[trial_idx - 1]:
    #         ts *= mapping[stimuli[trial_idx]]==td['A_correct'][trial_idx]
    #     else:
    #         Z_true[index:trial_idx] = numpy.where(ts)[0]
    #         index = trial_idx
    #         ts = numpy.ones(K);
    # Z_true[index:trial_idx] = numpy.where(ts)[0]
    Z_true = Z;
    ...
    # for trial_idx in np.arange(numberOfTrials): correctActions[trial_idx] = mapping[stimuli[trial_idx].astype(int), Z_true[trial_idx].astype(int)]
    ...
    # etaTracking            = np.zeros(numberOfTrials)
    betaStdTracking        = np.zeros(numberOfTrials)
    # etaStdTracking         = np.zeros(numberOfTrials)
    ...
    betaSamples                  = np.random.beta(betaPrior[0], betaPrior[1], numberOfThetaSamples)# np.ones(numberOfThetaSamples)#
    #etaSamples                   = np.random.beta(etaPrior[0], etaPrior[1], numberOfThetaSamples) # useful_functions.truncated_normal(etaPrior[0], etaPrior[1], 0, numberOfThetaSamples) #
    ...
    # #etaSamplesNew                = np.zeros(numberOfThetaSamples)
    # gammaSamplesNew              = np.zeros([numberOfThetaSamples,K])
    # taskSetSamplesNew            = np.zeros([numberOfThetaSamples,numberOfStateSamples], dtype=np.intc)
    # logThetaLksNew               = np.zeros(numberOfThetaSamples)
    # candidateTaskSetSamples      = np.zeros(numberOfStateSamples, dtype=np.intc)
    ...
    for T in range(numberOfTrials):
        ...
        # print(np.asarray([np.sum(currentTaskSetSamples==i) for i in range(K)])*1./(numberOfThetaSamples*numberOfStateSamples))
        ...
        if essList[T] < coefficient * numberOfThetaSamples and acceptance_list[-1] > 0.05:
             ...
             #     logThetaWeights[theta_idx] = 0;
             #     betaSamples[theta_idx] = betaCandidate;
             #     gammaSamples[theta_idx] = gammaCandidate;
        ...
        temperatureTracking[T]    = np.mean(currentNoises) #np.sum(normalisedThetaWeights * currentTemperatures)
        temperatureStdTracking[T] = np.std(currentNoises)
        #temperatureStdTracking[T] = np.sum(normalisedThetaWeights * (currentTemperatures - temperatureTracking[T])**2)

        #Update performance
        #if (actions[T] == mapping[stimuli[T].astype(int), Z_true[T].astype(int)]):
        # if (actions[T] == td['A_correct'][T]):
        if K == 2:
        ...
        if show_progress:
            ...
            plt.subplot(3,2,2)
            #plt.plot(temperatureTracking[:T], 'b'); plt.hold(True)
            ...
            # plt.plot(x, normlib.pdf(x, etaTracking[T], etaStdTracking[T]), 'b'); plt.hold(True)
            # plt.plot([etaTracking[T], etaTracking[T]], plt.gca().get_ylim(),'b', linewidth=2)
