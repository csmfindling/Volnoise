        ...
        for (int t = 0; t < nTrials; ++t) {
            ...
            for (int n_sample = 0; n_sample < N_samples; ++n_sample) {
                ...
                if (t > 0)
                {
                    ...
                    //print(descendant.get_mu());
                    descendant.update_l_deterministic(volatility);
                    //print(descendant.get_l());                    
                }
                descendant.verification();
...
//                            std::cout << t << std::endl;
                    ...
                    if ((lmax > 0.5) && (imax != 0)) {
                        ...
                        { // probe set becomes active task set
                            ...
                            if (descendant.get_monTS_size() >= mon_size + 2) {
                                ...
                                //std::cout << "out of memory - remove oldest task set" << std::endl;
                            }
                        }
                        descendant.set_state(1);
                        ...
                        //print(descendant.get_l());
                        //print(descendant.get_mu());
                        //print(descendant.get_monTS());
                        //print(descendant.get_ltmTS());
                        //cout << descendant.get_actorTS()<< endl;
                        //if (emergeTS > descendant.get_nTS()) {
                        //    throw std::invalid_argument( "emergeTS does not have correct value" );
                        //}
                    }
                    else
                ...
                if (cur_act >= 0) {
                    ...
                    *(rewards + t) = cur_rew;
                    //throw std::invalid_argument( "no action mentioned" );
                }
