        ...
        for (int t = 0; t < nTrials; ++t) {
            ...
            for (int n_sample = 0; n_sample < N_samples; ++n_sample) {
                ...
                if (t > 0)
                {
                    ...
                    double err = descendant.get_err(volatility, linear_);
                    
                    if (err > 0) {
                        gamma_sample = descendant.get_sampled_l(err, generator);
                        descendant.set_l_monTS(gamma_sample);
                        descendant.verification();
                    }
                    else
                    {
                        descendant.set_l_monTS_mu();
                    }
