...
double particle::get_err(double volatility, double linear)
{
    ...
    double err = 0;
    for (int idx = 0; idx < get_monTS_size(); ++idx) {
        err += std::abs(mu[monTS[idx]] - l[monTS[idx]]);// * 5 / get_monTS_size()
    }
    //std::cout << err << std::endl;
    if ((volatility>0)&&(linear>0))
    {
        err = err*volatility + linear;
    }
...
{
    if ((reward_lh.size() != get_monTS_size())||(bay_p.size()/12 != l.size())) {
  ...
  /*  for (int act_idx = 0; act_idx < nA; ++act_idx)
    ...
    for (int act_idx = 0; act_idx < nA; ++act_idx)
    {
        for (int stim_idx = 0; stim_idx < nS ; ++stim_idx)
        {
            ...
            {
                int idx_ts = ltmTS[idx];
                //ltm_avg_bay[stim_idx * nA + act_idx] += bay_p[idx_ts * nA * nS + stim_idx * nA + act_idx];
