...
{
    // Define discrete distribution
    boost::random::discrete_distribution<>  distribution(probabilities, probabilities + dimension);
    return distribution(generator);
}
...
void stratified_resampling(double uniformSample, double weights[], int dim, int* ancestorsIndexes)
{
    ...
    //vector<double> cumSum(dim);
    //partial_sum(&weights[0], &weights[0] + dim, cumSum.begin());
    double cumSum = weights[0] * (double)dim;
    //cumSum               = cumSum * (double)dim;
    int index            = 0;
    for (int i = 0; i < dim; ++i) {
        while (cumSum < uniformSample) { ++index; cumSum += (weights[index] * (double)dim);}
