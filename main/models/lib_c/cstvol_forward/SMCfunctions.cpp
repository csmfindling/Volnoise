...
/*        std::vector<double> gamma(K);
        ...
        for (int theta_idx = 0; theta_idx < numberOfThetaSamples; ++theta_idx) {
            ...
            if (t > 0) {
                ...
                //stratified_resampling(distribution(generator), weightsList + theta_idx * numberOfStateSamples, numberOfStateSamples, &ancestorsIndexes[0]);
                ...
                for (int traj_idx = 0; traj_idx < numberOfStateSamples; ++traj_idx) {
                    ...
                    else
                    {
                        ...
                        //isNotEqual_timesVector_normalise(mapping, currentStimulus, numberOfStimuli, K, mapping[currentStimulus * K + ancestor], gammaList + theta_idx * K, &gammaAdaptedProba[0]);
                        index          = 0;
