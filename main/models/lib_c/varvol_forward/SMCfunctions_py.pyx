...
cdef extern from "SMCfunctions.hpp" namespace "smc":
    ...
    # void guidedUpdateStep(double* logThetaWeights, int* currentStateRes, double* currentTauRes, double* gammaList, double* betaList, double* nuList, double tauDefault, int t, \
...
# def guidedUpdateStep_c(np.ndarray[double, ndim=1, mode="c"] logThetaWeights not None, np.ndarray[int, ndim=2, mode="c"] currentStateRes not None, \
...
#     guidedUpdateStep(&logThetaWeights[0], &currentStateRes[0,0], &currentTauRes[0,0], &gammaList[0,0], &betaList[0], &nuList[0], tauDefault, t, &ancestorsStateList[0,0], &ancestorsTauList[0,0], \
