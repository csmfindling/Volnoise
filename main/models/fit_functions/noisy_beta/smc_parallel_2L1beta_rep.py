...
def smc(actions, rewards, parameters, nb_samples):
	...
	for t_idx in range(T):
		...
		if t_idx > 0:
			...
			repetition_      = np.expand_dims((2 * actions[:,t_idx - 1][np.newaxis].T - 1) * (parameters[:,4][np.newaxis].T * 6 - 3.), -1)

		log_p_act      = np.transpose(np.transpose(- np.logaddexp(0., -particles/parameters[:,0][np.newaxis].T - repetition_).T * (1 - actions[:,t_idx]) \
											- np.logaddexp(0., particles/parameters[:,0][np.newaxis].T + repetition_).T * actions[:,t_idx]).T * (actions[:,t_idx] != -1))
