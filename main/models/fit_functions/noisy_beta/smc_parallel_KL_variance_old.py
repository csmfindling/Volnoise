...
def smc(actions, rewards, params):
	...
	for t_idx in range(T):
		...
		if t_idx > 0:
			...
			average_noise[:] = mu + distances * lambdaa
			...
			variances_beta   = np.square(np.sqrt(variances_beta) + np.random.rand(average_noise.shape[0], average_noise.shape[1]) * average_noise)
