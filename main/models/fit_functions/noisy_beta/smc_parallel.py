	...
	for t_idx in range(T):
		...
		if t_idx > 0:
			...
			distances[:] = np.sum(np.transpose(np.abs( \
									  		np.power(x_range.T, noisefree_update[:,:,0].T - 1) \
													* np.power((1 - x_range).T, noisefree_update[:,:,1].T - 1) \
															/ beta_func(noisefree_update[:,:,0].T, noisefree_update[:,:,1].T) * dx \
											- np.power(x_range.T, anc_particles[:,:,0].T - 1) \
													* np.power((1 - x_range).T, anc_particles[:,:,1].T - 1) \
															/ beta_func(anc_particles[:,:,0].T, anc_particles[:,:,1].T)  * dx \
										  ) \
									) \
							  , axis = -1)
