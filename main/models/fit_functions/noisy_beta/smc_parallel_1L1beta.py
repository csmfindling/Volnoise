...
def L1(count1, count2, par_confirmed, par_disconfir):
	...
	return du * (confirmed * 10. * par_confirmed + disconfir * 10.* par_disconfir)
...
def smc(actions, rewards, parameters, nb_samples):
	...
	for t_idx in range(T):
		...
		if t_idx > 0:
			...
			distances[:]     = L1(anc_particles, noisefree_update, parameters[:,1][np.newaxis].T, parameters[:,1][np.newaxis].T)
			#print(distances.mean())
			sampled_noise    = np.random.beta(1., distances) 
