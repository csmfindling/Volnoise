...
def smc(actions, rewards, parameters, nb_samples = 2000):
	...
	for t_idx in range(T):
		...
		if t_idx > 0:
			...
			precision_beta   = noisefree_update[:,0] + noisefree_update[:,1] - np.random.exponential(average_noise)

			particles[:,0] = np.maximum(means_beta * precision_beta, 1)
			particles[:,1] = np.maximum(precision_beta - means_beta * precision_beta, 1)
