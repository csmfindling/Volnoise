...
def smc(actions, rewards, parameters):
	...
	ancestors         = np.ones([nb_settings, nb_param, nb_samples], dtype = np.int)
	log_inc_marglkd   = 0
	anc_particles     = np.zeros([nb_settings, nb_param, nb_samples, 2])
	noisefree_update  = np.zeros([nb_settings, nb_param, nb_samples, 2])
	...
	for t_idx in range(T):
		...
		if t_idx > 0:
			...
			distances[:] = np.sum((noisefree_update - anc_particles)/anc_particles, axis=-1)

			average_noise[:] = 1./parameters[:,2][np.newaxis].T + distances/parameters[:,1][np.newaxis].T
			particles[:]     = np.maximum((1 - np.random.rand(particles.shape[0], particles.shape[1], particles.shape[2], particles.shape[3]) * 1/np.expand_dims(average_noise, -1)) * noisefree_update, 1)

			p_0_rewarding  = particles[:,:,:,0]/(particles[:,:,:,0] + particles[:,:,:,1])
			p_1_rewarding  = 1 - p_0_rewarding

		else:
			p_0_rewarding, p_1_rewarding = np.zeros([nb_settings, nb_param, nb_samples]) + 0.5, np.zeros([nb_settings, nb_param, nb_samples]) + 0.5
		...
		if np.any(np.isnan(log_inc_marglkd)):
			assert(False)
