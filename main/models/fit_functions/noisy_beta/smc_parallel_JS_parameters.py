...
def smc(actions, rewards, parameters):
	...
	for t_idx in range(T):
		...
		if t_idx > 0:
			...
			noise0             = np.random.normal(0, average_noise)
			noise1             = np.random.normal(0, average_noise)
			particles[:,:,:,0] = np.maximum(noisefree_update[:,:,:,0] - noise0 * (noise0 > 0), 1)
			particles[:,:,:,1] = np.maximum(noisefree_update[:,:,:,1] - noise1 * (noise1 > 0), 1)			
