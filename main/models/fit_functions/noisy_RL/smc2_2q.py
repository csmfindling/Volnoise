...
from noisy_RL.smc2 import smc_c
...
# actions must be between 0 and 1

# actions = td['A_chosen']; show_progress = False; apply_cst_noise = 1; beta_softmax=-1; apply_guided=0; apply_weber=1; temperature=1; apply_inertie=1;
def smc2(actions, rewards, idx_blocks, choices, subj_idx, show_progress, apply_cst_noise, apply_inertie, apply_weber, beta_softmax, temperature):
	
	assert(2 not in actions); assert(0 in actions); assert(1 in actions); assert(apply_cst_noise==0 or apply_cst_noise==1); assert(apply_weber==0 or apply_weber==1)
	...
	if apply_cst_noise:
		samples[:, 3]      = np.random.rand(n_theta) * upp_bound_eta
	...
	# noise variables 
	noise_amount_descendants = np.zeros([n_theta, N_samples, 2])
	noise_amount_ancestors   = np.zeros([n_theta, N_samples, 2])
	...
	noises_new          = np.zeros([n_theta, N_samples, 2])
	...
	noises_descen_p     = np.zeros([N_samples, 2])
	noises_ancest_p     = np.zeros([N_samples, 2])

	# history of samples
	noisy_history      = np.zeros([T,2])
	std_noisy_history  = np.zeros([T,2])
	...
	for t_idx in range(100):
		...
		smc_c.smc_update_2q_c(log_lkd, logThetalkd, noisy_descendants, noise_amount_descendants, noisy_ancestors, noise_amount_ancestors, weights_norm, logThetaWeights, ancestorsIndexes, samples, \
															prev_action, actions, prev_rew, t_idx, apply_cst_noise, apply_inertie, apply_weber, 1, temperature)
		...
		noisy_history[t_idx]     = np.sum((normalisedThetaWeights * np.sum(np.transpose(weights_norm.T * noisy_descendants.T), axis=1).T), axis=1)
		...
		if (essList[t_idx] < coefficient * n_theta):
			...
			for theta_idx in range(n_theta):
				...
				while True:
					...
					if (not sample_beta) and apply_cst_noise:
					...
					if apply_cst_noise:
						if sample_p[0] > 0 and sample_p[0] < 1 and sample_p[1] > 0 and sample_p[1] <= upp_bound_beta \
														and sample_p[2] > 0 and sample_p[2] < upp_bound_eps and sample_p[3] > 0 and sample_p[3] < upp_bound_eta:
							if apply_inertie:
								if sample_p[-1] > 0 and sample_p[-1] < 1:
									break
							else:
								break
					else:
				...
				# Launch SMC
				logmarglkd_p = smc_c.smc_2q_c(state_candidates, noises_descen_p, state_candidates_a, noises_ancest_p, weights_candidates, sample_p, ancestors_indexes_p, \
				...
				if np.log(np.random.rand()) < logAlpha:
					...
					noises_new[theta_idx]      = noises_descen_p
				else:
					...
					noises_new[theta_idx]      = noise_amount_descendants[idx_traj]
			...
			noise_amount_descendants[:]   = noises_new
		...
		if show_progress and t_idx % 10:
			...
			# plt.fill_between(range(t_idx), noisy_history[:t_idx,0] -  noisy_history[:t_idx,1] - 2*std_noisy_history[:t_idx], noisy_history[:t_idx,0] -  noisy_history[:t_idx,1] + 2*std_noisy_history[:t_idx], color='r', alpha=.5); plt.hold(True)
			plt.hold(False)
			plt.xlabel('trials')
	...
	return [samples, noisy_history, std_noisy_history, acceptance_list, normalisedThetaWeights, logThetalkd,  marg_loglkd]
