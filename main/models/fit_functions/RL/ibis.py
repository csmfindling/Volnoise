...
# actions = td['A_chosen']; tau=td['tau']; rewards = td['reward']; subj_idx=subj_idx; show_progress = False; beta_softmax=-1; apply_rep_bias=0; temperature=1; model_id=0
def ibis(actions, rewards, tau, subj_idx, apply_rep_bias, show_progress = True, temperature = True, model_id = 0):
	...
	for t_idx in range(T):
		...
		if ess < coefficient * nb_samples:
			...
			#samples_temp    = np.concatenate((samples[:,np.array([x_coor[t_idx]])], samples[:,n_alpha:]), axis=1)
			...
			for n_idx in range(nb_samples):
				...
				# sample_p                = samples[idx_traj]
				# sample_p[x_coor[t_idx]] = sample_p_temp[0]
				# sample_p[n_alpha:]      = sample_p_temp[1:]
