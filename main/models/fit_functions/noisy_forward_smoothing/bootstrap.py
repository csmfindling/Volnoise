...
# td = pickle.load(open('../../../../data/python/td_volnoise_subj_10_run_0_session_0.pkl')); show_progress=True; numberOfStateSamples=200; latin_hyp_sampling =True;  epsilon_softmax=0.; noise_inertie = 0.; numberOfThetaSamples=200; coefficient = .5; beta_softmax=1.; lambda_noise=1.; eta_noise = .05; numberOfBetaSamples=20 ;alpha=0.; 
def SMC2(td, beta_softmax=1., lambda_noise=.4, eta_noise=.1, epsilon_softmax=0., noise_inertie = 0., numberOfStateSamples=200, numberOfThetaSamples=200, numberOfBetaSamples=20, coefficient = .5, latin_hyp_sampling=True):
    ...
    all_betaWeights   = np.zeros([numberOfTrials, numberOfBetaSamples])
    all_betaAncestors = np.zeros([numberOfTrials, numberOfBetaSamples])
    ...
    all_temperatures              = np.zeros([numberOfTrials, numberOfBetaSamples])
    all_distances                 = np.zeros([numberOfTrials, numberOfBetaSamples])
    all_beliefprobabilities       = np.zeros([numberOfTrials, numberOfBetaSamples, K])
    all_PE                        = np.zeros([numberOfTrials, numberOfBetaSamples])
    # Loop over trials
    for T in range(numberOfTrials):
        ...
        for beta_idx in range(numberOfBetaSamples):
            ...
            all_distances[T, beta_idx] = distances[0]
            ...
            all_PE[T, beta_idx]        = 1 - np.sum(normalisedThetaWeights[beta_idx] * np.mean(weightsList, axis=-1))
        ...
        # save
        all_temperatures[T]        = temperatures
        all_betaAncestors[T]       = betaAncestors
        all_betaWeights[T]         = betaWeights
        all_beliefprobabilities[T] = tsProbability

    elapsed_time = time.time() - start_time_multi

    return log_proba_, all_betaWeights, all_betaAncestors, all_temperatures, all_distances, all_beliefprobabilities, all_PE


# plt.plot(PE[1:])
# plt.plot(td['Z'][:-1] * 0.6 + 0.2, 'o', color='red', markersize=5)
# trap = (td['Z'] == td['A_chosen']) * (td['reward'] == 0) + (td['Z'] != td['A_chosen']) * (td['reward'] == 1)
# plt.plot(trap[:-1] * 0.5, 'o', color='green', markersize=5)
