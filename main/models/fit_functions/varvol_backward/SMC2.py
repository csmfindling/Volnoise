...
def SMC2(td, show_progress=False, numberOfStateSamples=1000, numberOfThetaSamples=1000, coefficient = .5):
    ...
    for T in range(numberOfTrials):
        ...
        if (essList[T] < coefficient * numberOfThetaSamples) and (acceptance_list[-1] > 0.05):
            ...
            for theta_idx in range(numberOfThetaSamples):
                ...
                # priorsLogRatio                 = useful_functions.log_invgamma_pdf(nuCandidate, nuPrior[0], nuPrior[1]) + useful_functions.log_beta_pdf(betaCandidate, betaPrior[0], betaPrior[1]) + useful_functions.log_dirichlet_pdf(gammaCandidate, gammaPrior) - \
                #                                     useful_functions.log_invgamma_pdf(nuSamples[idx_traj], nuPrior[0], nuPrior[1]) - useful_functions.log_beta_pdf(betaSamples[idx_traj], betaPrior[0], betaPrior[1]) - useful_functions.log_dirichlet_pdf(gammaSamples[idx_traj], gammaPrior)
        ...
        # volStdTracking[T] = np.sum(currentNormalisedThetaWeights * np.sum((currentBootstrapTauSamples - volTracking[T])**2, axis = 1))
        ...
        if show_progress:
            ...
            #plt.plot(td['tau'][:T], 'b--', linewidth=2)
    ...
    return [td, nuSamples, nuTracking, nuStdTracking, volTracking, volStdTracking, betaSamples, betaTracking, tsProbability, betaStdTracking, gammaSamples, countPerformance, actions, acceptance_list, essList, time_list, elapsed_time]
